...
namespace Taumis.Alpha.WinClient.Aurora.Modules.Reports.DebtAndFine.Views.List
{
    public class ListViewPresenter : BaseReportForGridPresenter<IListView, EmptyReportParams>
    {
        ...
        public void FillApartmentsList()
        {
            ...
            using (Entities _db = new Entities())
            {
                var _aps = _db.Customers.Where(c => c.Buildings.ID == _bID).Select(c => c.Apartment).ToList();
            ...
            IEnumerable<int> waterSrvIds)
        {
            ...
            if (r.Key.Year < YEAR)
            {
                _days = (now - r.Key.AddDays(PAY_DAY)).Days;
                _days300 = _days130 = 0;
                _fine = _debt * _days * fineRate / 100 / FINE_300;
            }
            else
            {
                _days300 = (now - r.Key.AddDays(PAY_DAY + DateTime.DaysInMonth(r.Key.Year, r.Key.Month))).Days;
                ...
                _days130 = (now - r.Key.AddDays(PAY_DAY + DAYS_300)).Days;
                if(_days130 < 0)
                {
                    _days130 = 0;
                }
