...
using DBItem = Taumis.Alpha.DataBase.DecFormsUploads;
using DomItem = Taumis.Alpha.Infrastructure.Interface.BusinessEntities.Doc.DecFormsUpload;
...
{
    public class DecFormsUploadDataMapper : BaseDataMapper<DomItem, DBItem>, IDataMapper
    {
        protected override DBItem BusinessToService(DomItem domObj)
        {
            ...
            using (Entities _entities = new Entities())
            {
                ...
                {
                    _dbItem = new DBItem();
                    _entities.AddToDecFormsUploads(_dbItem);
                }
                else
                {
                    int _id = int.Parse(domObj.ID);
                    _dbItem = _entities.DecFormsUploads.First(x => x.ID == _id);
                }
        ...
        protected override DomItem ServiceToBusiness(IDomainObject obj)
        {
            ...
            using (Entities _entities = new Entities())
            {
                DBItem _dbItem = _entities.DecFormsUploads
                    .Include("Author")
                    .First(x => x.ID == _id);
        ...
        public override bool checkExistance(IDomainObject obj)
        {
            ...
            using (Entities _entities = new Entities())
            {
                _result = null != _entities.DecFormsUploads.FirstOrDefault(p => p.ID == _domainId);
            }
